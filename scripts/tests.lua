...
local function with_captured_notifications(fn)
  ...
  if renderer then
    renderer.on_cells_changed = function(force_index, surface_index, changed_set, new_region_id) ---@diagnostic disable-line: duplicate-set-field
      table.insert(calls.cells, { force_index = force_index, surface_index = surface_index, changed = changed_set, region = new_region_id })
    end
...
local function test_tile_to_cell()
  ...
  local x1,y1 = backend.tile_to_cell(c.force_index, c.surface_index, tx1, ty1)
  assert_eq(x1, 0, "tile_to_cell(0,0) x")
  assert_eq(y1, 0, "tile_to_cell(0,0) y")

  local tx2 = 31.9 ---@type double
  local ty2 = 31.9 ---@type double
  local x2,y2 = backend.tile_to_cell(c.force_index, c.surface_index, tx2, ty2)
  assert_eq(x2, 0)
  assert_eq(y2, 0)

  local tx3 = 32.0 ---@type double
  local ty3 = 0.0 ---@type double
  local x3,y3 = backend.tile_to_cell(c.force_index, c.surface_index, tx3, ty3)
  assert_eq(x3, 1)
  assert_eq(y3, 0)
...
local function test_rectangle_fill_undo_redo()
  ...
  local calls = with_captured_notifications(function(calls)
    backend.fill_rectangle(c.player_index, c.force_index, c.surface_index, region.id, lt, rb)
  end)
...
local function test_line_draw_orientation()
  reset_backend()
  local c = ctx()
  local z = backend.add_region(c.force_index, c.player_index, "ZL", {r=0,g=1,b=0,a=1})

  -- vertical (width<=height)
  do
    local g = backend.get_grid(c.force_index, c.surface_index)
    backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=32,height=32,x_offset=g.x_offset,y_offset=g.y_offset}, {reproject=false})
  end
  ...
  do
    local g = backend.get_grid(c.force_index, c.surface_index)
    backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=64,height=32,x_offset=g.x_offset,y_offset=g.y_offset}, {reproject=false})
  end
...
local function test_set_grid_notifications_and_reproject()
  ...
  local calls2 = with_captured_notifications(function()
    local g = backend.get_grid(c.force_index, c.surface_index)
    backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=16,height=16,x_offset=g.x_offset,y_offset=g.y_offset}, {reproject=true})
  end)
  cells = get_cells(c.force_index, c.surface_index)
...
local function test_tile_to_cell_offsets_and_negative()
  ...
  local c = ctx()
  local g = backend.get_grid(c.force_index, c.surface_index)
  backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=32,height=32,x_offset=16,y_offset=32}, {reproject=false})

  local x0,y0 = backend.tile_to_cell(c.force_index, c.surface_index, 16.0, 32.0)
  assert_eq(x0, 0, "offset origin x")
  assert_eq(y0, 0, "offset origin y")

  local xm,ym = backend.tile_to_cell(c.force_index, c.surface_index, 15.9, 31.9)
  assert_eq(xm, -1, "negative boundary x")
  assert_eq(ym, -1, "negative boundary y")

  local xn,yn = backend.tile_to_cell(c.force_index, c.surface_index, -1.0, -1.0)
  assert_eq(xn, -1, "negative tile x with offset")
  assert_eq(yn, -2, "negative tile y with offset")

  local xp,yp = backend.tile_to_cell(c.force_index, c.surface_index, 48.0, 64.0)
...
local function test_alt_mode_line_orientation()
  ...
  reset_backend()
  local c = ctx()
  local z = backend.add_region(c.force_index, c.player_index, "ALT", {r=0,g=0.5,b=1,a=1})
  do
    local g = backend.get_grid(c.force_index, c.surface_index)
    backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=32,height=32,x_offset=g.x_offset,y_offset=g.y_offset}, {reproject=false})
  end
  ...
  reset_backend()
  c = ctx()
  z = backend.add_region(c.force_index, c.player_index, "ALT2", {r=0.2,g=0.8,b=0.4,a=1})
  do
    local g = backend.get_grid(c.force_index, c.surface_index)
    backend.set_grid(c.force_index, game.surfaces["nauvis"].index, c.player_index, {width=64,height=32,x_offset=g.x_offset,y_offset=g.y_offset}, {reproject=false})
  end
...
remote.add_interface("region_planner_demo", {
  ...
  paint_sample = function(force_index, surface_index)
    ...
    if #ids < 3 then
      ...
      ids = {}
      for id, z in pairs(storage.zp.forces[f_idx].regions) do if id ~= 0 then ids[#ids+1] = id end end
      table.sort(ids)
    end

    local z1, z2, z3 = ids[1], ids[2], ids[3]
    -- Paint 2x2 cells near origin
    local W = backend.get_grid(f_idx, s_idx).width
    local H = backend.get_grid(f_idx, s_idx).height
