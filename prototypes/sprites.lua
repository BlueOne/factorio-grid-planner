...
local filename = "__grid-planner__/graphics/base/empty-square.png"
for x = 0, 2 do
    data:extend{{
        type = "sprite",
        name = "gp-empty-square-" .. x,
        filename = filename,
        size = 512,
        x = x * 512,
...
local filename = "__grid-planner__/graphics/base/corners.png"
for x = 0, 2 do
    for y = 0, 3 do
        data:extend{{
            type = "sprite",
            name = ("gp-corner-%d-%d"):format(x, y),
            filename = filename,
            size = 256,
            x = x * 256,
            y = y * 256,
...
-- example "gp-up-dark-16", "gp-up-mid-16", "gp-up-light-16", "gp-up-dark-64", etc
local names = {"up", "down", "right", "left", "visibility", "edit"}
local resolutions = {16, 32, 64}
for _, resolution in pairs(resolutions) do
    for i, name in pairs(names) do
        add_icon_row("gp-" .. name, "-" .. resolution, i - 1, "__grid-planner__/graphics/icons/icons-" .. resolution .. ".png", resolution)
    end
end
...
data:extend({
    ...
    },
    {
        type = "sprite",
        name = "gp-mod-icon",
        filename = "__grid-planner__/graphics/icons/mod-icon.png",
        size = 64,
        flags = {"gui-icon"},
    }
})
